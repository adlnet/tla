(function(ADL){
    ...
    XAPIYoutubeStatements = function() {
        ...
        this.playVideo = function(event){
            var stmt = playVideo(event.currentTarget.currentTime);
            var e = "playing"
            ADL.XAPIYoutubeStatements.onStateChangeCallback(e, stmt);
        }
        ...
        this.pauseVideo = function(event){
            if(!event.target.ended ){
                var stmt = pauseVideo(event.currentTarget.currentTime);
                var e = "playing"
                ADL.XAPIYoutubeStatements.onStateChangeCallback(e, stmt);
            }
        }
        ...
        this.completedVideo = function(event){
            var ISOTimeString = "PT" + precisionRound(event.currentTarget.currentTime, 2).toString() + "S";
            var stmt = completeVideo(ISOTimeString);
            var e = "playing"
            ADL.XAPIYoutubeStatements.onStateChangeCallback(e, stmt);
        }
